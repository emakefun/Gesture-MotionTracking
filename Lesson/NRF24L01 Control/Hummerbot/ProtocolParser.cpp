...
{
    // DEBUG_LOG(DEBUG_LEVEL_INFO, "ParserPackage start\n");
    ...
    if (recflag) {
        ...
        recv->start_code = *m_pHeader;
        recv->len = *(m_pHeader + 1);
        for (int i = 1; i < m_PackageLength - 3; i++) {
            check_sum += *(m_pHeader + i);
        }
        if ((check_sum & 0xFFFF) != GetCheckSum()) {
              DEBUG_ERR("check sum error \n");
              for (int i = 0; i < m_PackageLength; i++) {
                  DEBUG_LOG(DEBUG_LEVEL_ERR, "0x%x ", *(m_pHeader + i));
              }
              return false ;
	    }
    	recv->function = *(m_pHeader + 4);;
    	recv->data = m_pHeader + 5;;
    	protocol_data_len = m_PackageLength - 8;
    	recv->end_code = *(m_pHeader + m_RecvDataIndex);
      // DEBUG_LOG(DEBUG_LEVEL_INFO, "ParserPackage end \n");
    	return true;
	}
  return false;
}
