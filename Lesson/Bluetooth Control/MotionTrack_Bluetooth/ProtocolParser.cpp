...
ProtocolParser::ProtocolParser(byte header = PROTOCOL_START_CODE, byte endcode = PROTOCOL_END_CODE, p_read pread = NULL, p_write pwrite = NULL)
{
    m_recv_flag = false;
...
bool ProtocolParser::RecevData(byte *data, size_t len)
{
    ...
    for (int i=0; i < len; i++) {
        if (avilable == false && data[i] == m_StartCode) {
            m_pHeader = buffer;
            *m_pHeader++ = data[i];
            m_RecvDataIndex = 0;
            avilable = true;
        ...
        if (avilable) {
            if (data[i] == m_EndCode && buffer[1] == m_RecvDataIndex) {
              if (m_RecvDataIndex > buffer[1]) {
                m_pHeader = buffer;
                avilable = false;
                recflag = false;
